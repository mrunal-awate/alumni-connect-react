...
const Mentorship = () => {
  ...
  useEffect(() => {
    ...
    const fetchMentors = async () => {
      try {
        // Alumni table actual columns: name, email, year, branch, role, verified
        const { data: alumniData } = await supabase
          .from("alumni")
          .select("*")
          .eq("verified", true);
        ...
        // Mentorship table (admin / self-registered mentors)
        const { data: mentorshipData } = await supabase
          .from("mentorship")
          .select("*");

        // Map alumni using ACTUAL columns only: name, email, year, branch
        const alumniMentors = (alumniData || []).map((a, index) => {
          // year is "2011-2012" or "2022-23" → extract end year cleanly
          let batchYear = null;
          if (a.year) {
            const parts = a.year.split("-");
            if (parts.length === 2) {
              const endPart = parts[1].trim();
              batchYear = endPart.length === 2 ? parts[0].substring(0, 2) + endPart : endPart;
            }
          }

          return {
            id: "alumni_" + (a.id || index),
            source: "alumni",
            name: a.name || "Unknown",
            batch_year: batchYear,                     // e.g. "2012"
            job_title: null,
            company: null,
            experience: null,
            expertise: a.branch || null,               // e.g. "Computer Engineer"
            bio: null,
            tags: a.branch ? a.branch : null,          // show branch as tag
            email: a.email || null,
            availability: "Available",
            avatar_color: getAvatarColor("alumni", a.name),
          };
        ...
        // Map mentorship table (admin entries)
        const mentorshipMentors = (mentorshipData || []).map((m) => ({
          id: "mentor_" + m.id,
          source: "admin",
          name: m.name || "Unknown",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredMentors.map((mentor) => (
              ...
              >
                ...
                {/* Job + Company — OR show email as fallback for alumni with no job data */}
                {mentor.job_title ? (
                  <div className="flex items-start gap-1.5 mb-1">
                    <span className="text-gray-400 mt-0.5">💼</span>
                    <div>
